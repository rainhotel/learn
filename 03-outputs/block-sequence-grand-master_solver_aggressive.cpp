...
public:
    ...
    int worst_score() const {
        ...
        return _items.back().heuristic_score;
    }
...
static ComponentSearchResult search_component(const Board& board,
    ...
    std::sort(starts.begin(), starts.end(), [&](int a, int b) {
        ...
        if (bomb_level && ca.is_bomb() != cb.is_bomb()) return ca.is_bomb() && !cb.is_bomb();
        if (board.level >= 3 && ca.is_wildcard() != cb.is_wildcard()) return ca.is_wildcard() && !cb.is_wildcard();
    ...
    auto push_cell = [&](int idx) {
        ...
        visited[idx] = 1;
        path.push_back(idx);
        if (board.at(idx / N, idx % N).is_wildcard()) ++wildcards_in_path;
    ...
    auto aggression_bonus = [&](int len, int blast_cells, int bombs_now, int wildcards_now) {
        int bonus = 0;
        ...
        if (board.level <= 2) {
            ...
            bonus += blast_cells * 2;
            bonus += bombs_now * 34;
            bonus += wildcards_now * 4;
            if (bombs_now > 0 && blast_cells >= 6) bonus += 20;
            if (bombs_now == 0 && len <= 3) bonus -= 16;
        }
    ...
    auto dfs = [&](auto& self, int cur_idx) -> void {
        ...
        if (len >= 2) {
            ...
            int heuristic_score = score + aggression_bonus(len, bonus_cells, bombs_in_path, wildcards_in_path);
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            if (bomb_level && board.at(nr, nc).is_bomb()) priority -= 52;
            if (board.at(nr, nc).is_wildcard()) priority -= 18;
...
static std::vector<SearchStage> make_search_plan(const Board& board) {
    if (board.N == 12) {
        return {
            {{220000, 5400, 2200, 30, 8, 12}, 12, 95, 0.92},
            {{ 70000, 2000, 1050, 12, 4,  9},  5, 45, 0.68},
            {{ 18000,  700,  380,  6, 2,  6},  1,  0, 0.00},
        };
    }

    if (board.level <= 2) {
        return {
            {{290000, 6200, 2900, 32, 7, 14}, 18, 90, 0.96},
            {{ 90000, 2400, 1200, 12, 4, 10},  5, 45, 0.72},
            {{ 22000,  900,  500,  6, 2,  7},  1,  0, 0.00},
        };
    }

    return {
        {{240000, 5400, 2400, 34, 8, 14}, 14, 90, 0.90},
        {{ 80000, 2200, 1100, 14, 5, 10},  5, 45, 0.66},
        {{ 20000,  800,  420,  6, 2,  7},  1,  0, 0.00},
    };
}
