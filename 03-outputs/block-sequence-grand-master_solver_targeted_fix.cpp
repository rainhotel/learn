...
static int pure_color_structure_score(const Board& board) {
    if (board.level > 2) return 0;
    ...
    for (int r = 0; r < N; ++r) {
        for (int c = 0; c < N; ++c) {
            ...
            if (c + 1 < N && board.at(r, c + 1).color() == color) ++same_edges;
            if (r + 1 < N && board.at(r + 1, c).color() == color) ++same_edges;
        }
    }
    ...
    for (int idx = 0; idx < total; ++idx) {
        ...
        int color = board.at(idx / N, idx % N).color();
        int head = 0;
        int tail = 0;
        int size = 0;
        ...
        while (head < tail) {
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                int ni = nr * N + nc;
                if (seen[ni]) continue;
                if (board.at(nr, nc).color() != color) continue;
                seen[ni] = 1;
                q[tail++] = ni;
            }
        }
        ...
        if (size == 1) {
            ...
            component_value += path_score(size);
        }
    }

    return component_value + largest * 20 + same_edges * 8 - isolated * 6;
}
...
static bool better_candidate(const MoveCandidate& a, const MoveCandidate& b) {
    if (a.heuristic_score != b.heuristic_score) return a.heuristic_score > b.heuristic_score;
    if (a.score != b.score) return a.score > b.score;
    if (a.length != b.length) return a.length > b.length;
    if (a.blast_cells != b.blast_cells) return a.blast_cells > b.blast_cells;
...
class CandidatePool {
    ...
    static bool better_tuple(int heuristic_a, int score_a, int len_a, int blast_a, int bombs_a,
                             int heuristic_b, int score_b, int len_b, int blast_b, int bombs_b) {
        if (heuristic_a != heuristic_b) return heuristic_a > heuristic_b;
        if (score_a != score_b) return score_a > score_b;
        if (len_a != len_b) return len_a > len_b;
        if (blast_a != blast_b) return blast_a > blast_b;
...
public:
    ...
    bool worth_consider(int heuristic_score, int score, int length, int blast_cells, int bomb_count) const {
        if (_limit <= 0) return false;
        if (static_cast<int>(_items.size()) < _limit) return true;
        const MoveCandidate& worst = _items.back();
        return better_tuple(heuristic_score, score, length, blast_cells, bomb_count,
                            worst.heuristic_score, worst.score, worst.length, worst.blast_cells, worst.bomb_count);
    }

    void add_from_indices(const std::vector<int>& idx_path, int N, int score,
                          int heuristic_score, int bomb_count, int blast_cells) {
        int length = static_cast<int>(idx_path.size());
        if (!worth_consider(heuristic_score, score, length, blast_cells, bomb_count)) return;
        uint64_t h = hash_path_indices(idx_path);
        if (!_seen.insert(h).second) return;

        MoveCandidate cand;
        cand.score = score;
        cand.heuristic_score = heuristic_score;
        cand.length = length;
        cand.bomb_count = bomb_count;
        cand.blast_cells = blast_cells;
    ...
    void add_candidate(const MoveCandidate& cand, int N) {
        if (!worth_consider(cand.heuristic_score, cand.score, cand.length, cand.blast_cells, cand.bomb_count)) return;
...
static ComponentSearchResult search_component(const Board& board,
    ...
    bool pure_color_aggressive = (board.level <= 2);
    bool pure_color_connector_mode = pure_color_aggressive &&
                                     static_cast<int>(comp.cells.size()) >= 8;
    bool wildcard_bridge_level = (board.level == 3);
    ...
    std::sort(starts.begin(), starts.end(), [&](int a, int b) {
        ...
        if (pure_color_aggressive) {
            if (pure_color_connector_mode) {
        ...
        if (wildcard_bridge_level) {
            auto wildcard_rank = [&](int idx, const Cell& cell) {
                if (!cell.is_wildcard()) return 0;
                if (degree[idx] >= 3) return 3;
                if (degree[idx] == 2) return 2;
                return 1;
            };
            int ra = wildcard_rank(a, ca);
            int rb = wildcard_rank(b, cb);
    ...
    int bonus_cells = 0;
    int bombs_in_path = 0;
    int wildcard_bridge_cost = 0;
    int bfs_token = 0;

    auto chain_bias = [&](int len) {
        if (!pure_color_aggressive) return 0;
        int bias = pure_color_connector_mode ? len * 3 : len * 6;
        if (len >= 8) bias += pure_color_connector_mode ? 8 : 18;
        if (len >= 10) bias += pure_color_connector_mode ? 10 : 20;
        if (len <= 3) bias -= pure_color_connector_mode ? 8 : 16;
        return bias;
    };

    auto wildcard_cost = [&](int idx) {
        if (!wildcard_bridge_level) return 0;
        const Cell& cell = board.at(idx / N, idx % N);
        if (!cell.is_wildcard()) return 0;
        if (degree[idx] >= 4) return 24;
        if (degree[idx] == 3) return 16;
        if (degree[idx] == 2) return 6;
        return -4;
    };

    auto local_future_potential = [&](int end_idx, int len) {
        int r = end_idx / N;
        int c = end_idx % N;
        int exits = 0;
        int branch_mass = 0;
        for (int d = 0; d < 4; ++d) {
            ...
            for (int dd = 0; dd < 4; ++dd) {
                ...
                int nni = nnr * N + nnc;
                if (in_comp[nni] && !visited[nni]) ++next_deg;
            }
            branch_mass += next_deg;
        }

        int edge_depth = std::min({r, c, N - 1 - r, N - 1 - c});
        int remaining = static_cast<int>(comp.cells.size()) - len;

        if (pure_color_aggressive) {
            int potential = exits * 24 + branch_mass * 4 + std::min(remaining, 8) * 3;
            if (pure_color_connector_mode) potential += edge_depth * 10;
            return potential;
        }
        if (wildcard_bridge_level) {
            return exits * 18 + branch_mass * 3;
        }
        return 0;
    };
    ...
    auto push_cell = [&](int idx) {
        ...
        visited[idx] = 1;
        path.push_back(idx);
        wildcard_bridge_cost += wildcard_cost(idx);
    ...
    auto dfs = [&](auto& self, int cur_idx) -> void {
        ...
        if (len >= 2) {
            ...
            int heuristic_score = score + chain_bias(len) + local_future_potential(cur_idx, len) - wildcard_bridge_cost;
            pool.add_from_indices(path, N, score, heuristic_score, bombs_in_path, bonus_cells);
        }
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            int edge_depth = std::min({nr, nc, N - 1 - nr, N - 1 - nc});
            int priority = rem_deg * 100;
            if (pure_color_connector_mode) {
                priority = -rem_deg * 100 - edge_depth * 12;
            }
            if (bomb_level && board.at(nr, nc).is_bomb()) priority -= 35;
            if (wildcard_bridge_level && board.at(nr, nc).is_wildcard()) {
                if (degree[ni] >= 3) priority += 36;
                else if (degree[ni] == 2) priority += 10;
                else priority -= 8;
            } else if (board.at(nr, nc).is_wildcard()) {
                priority -= 12;
            }
            if (!pure_color_connector_mode) priority += edge_depth;
...
static int estimate_future_score(const Board& board,
    ...
    for (int i = 0; i < branch; ++i) {
        ...
        int shape = pure_color_structure_score(next_board);
        ...
        total += static_cast<int>(std::lround(stage.shape_weight * shape));
...
static std::vector<SearchStage> make_search_plan(const Board& board, int step) {
    if (board.level == 1) {
        return {
            step < 14 ? SearchStage{{360000, 7200, 3300, 38, 8, 16}, 22, 110, 1.02, 0.86, 0.18}
                      : step < 32 ? SearchStage{{330000, 6800, 3200, 36, 8, 15}, 21, 100, 0.94, 0.96, 0.10}
                                  : SearchStage{{280000, 6000, 2900, 34, 8, 14}, 18,  90, 0.80, 1.06, 0.02},
            step < 14 ? SearchStage{{110000, 2800, 1400, 14, 4, 10},  7,  50, 0.78, 0.92, 0.08}
                      : step < 32 ? SearchStage{{ 98000, 2500, 1280, 13, 4, 10},  6,  45, 0.66, 1.00, 0.04}
                                  : SearchStage{{ 80000, 2200, 1150, 12, 4,  9},  5,  40, 0.54, 1.08, 0.00},
            {{24000, 950, 520, 6, 2, 7}, 1, 0, 0.00, 1.00, 0.00},
        };
    }

    if (board.level == 2) {
        return {
            step < 10 ? SearchStage{{340000, 7000, 3250, 36, 8, 16}, 22, 100, 0.98, 0.92, 0.14}
                      : step < 28 ? SearchStage{{320000, 6500, 3050, 35, 8, 15}, 20,  95, 0.90, 1.00, 0.08}
                                  : SearchStage{{290000, 6000, 2850, 34, 8, 14}, 18,  90, 0.76, 1.10, 0.00},
            step < 10 ? SearchStage{{105000, 2700, 1360, 14, 4, 10},  7,  50, 0.74, 0.96, 0.06}
                      : step < 28 ? SearchStage{{ 95000, 2450, 1240, 13, 4, 10},  6,  45, 0.62, 1.02, 0.03}
                                  : SearchStage{{ 82000, 2200, 1140, 12, 4,  9},  5,  40, 0.50, 1.08, 0.00},
            {{24000, 950, 520, 6, 2, 7}, 1, 0, 0.00, 1.00, 0.00},
        };
    }
    ...
    if (board.level == 3) {
        return {
            step < 18 ? SearchStage{{240000, 5300, 2550, 28, 7, 13}, 14, 70, 0.62, 1.08, 0.00}
                      : SearchStage{{225000, 5000, 2400, 27, 7, 13}, 12, 70, 0.56, 1.14, 0.00},
            step < 18 ? SearchStage{{ 72000, 1950, 1080, 11, 4,  8},  4, 35, 0.40, 1.02, 0.00}
                      : SearchStage{{ 68000, 1850, 1020, 10, 4,  8},  4, 35, 0.34, 1.05, 0.00},
            {{18000, 700, 400, 5, 2, 6}, 1, 0, 0.00, 1.00, 0.00},
        };
    }

    if (board.level == 4) {
        return {
            {{190000, 4200, 2100, 24, 6, 12}, 10, 70, 0.78, 1.00, 0.00},
            {{ 60000, 1800,  950, 10, 4,  8},  4, 35, 0.58, 1.00, 0.00},
            {{ 15000,  600,  350,  5, 2,  6},  1,  0, 0.00, 1.00, 0.00},
        };
    }

    if (board.level == 5) {
        return {
            {{150000, 4000, 1700, 18, 5, 10}, 8, 70, 0.75, 1.00, 0.00},
            {{ 35000, 1200,  700,  8, 3,  7}, 3, 35, 0.55, 1.00, 0.00},
            {{ 10000,  450,  250,  4, 2,  5}, 1,  0, 0.00, 1.00, 0.00},
        };
    }
...
std::vector<std::pair<int,int>> find_best_path(const Board& board, int step) {
    std::vector<SearchStage> plan = make_search_plan(board, step);
    ...
    for (int i = 0; i < branch; ++i) {
        ...
        total += static_cast<int>(std::lround(plan.front().shape_weight * shape));
