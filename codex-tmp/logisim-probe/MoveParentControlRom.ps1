...
if ($comboStart -lt 0) { throw 'Cannot find combo circuit' }
$comboEnd = $text.IndexOf('</circuit>', $comboStart)
if ($comboEnd -lt 0) { throw 'Cannot find combo circuit end' }
...
if (!$romMatch.Success) { throw 'Cannot find combo ROM' }
$farRom = $romMatch.Value.Replace('loc="(330,140)"', 'loc="(1500,850)"')
...
$body = [regex]::Replace(
    $body,
    '<comp lib="4" loc="\((680),(350)\)" name="ROM">(?:(?!</comp>).)*?</comp>',
    '',
    [System.Text.RegularExpressions.RegexOptions]::Singleline)
$remove = @(
...
if ($body -notlike '*loc="(1500,850)" name="ROM"*') {
    $new = @(
        $farRom,
        '<comp lib="0" loc="(1360,850)" name="Tunnel"><a name="facing" val="east" /><a name="width" val="13" /><a name="label" val="OCInput" /><a name="labelfont" val="Dialog plain 12" /></comp>',
        '<comp lib="0" loc="(1390,890)" name="Constant"><a name="facing" val="east" /><a name="width" val="1" /><a name="value" val="0x0" /></comp>',
        '<wire from="(1390,890)" to="(1410,890)" />',
        '<comp lib="0" loc="(1530,850)" name="Tunnel"><a name="facing" val="west" /><a name="width" val="22" /><a name="label" val="ControlBus" /><a name="labelfont" val="Dialog plain 12" /></comp>',
        '<wire from="(1500,850)" to="(1530,850)" />'
    ) -join ''
    $insertBefore = '<comp loc="(1150,250)" name="◇硬布线控制器组合逻辑单元">'
    $index = $body.IndexOf($insertBefore)
    if ($index -lt 0) { throw 'Cannot find insertion point before combo instance' }
