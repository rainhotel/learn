...
| 长尾、负载模型、tag 基数和线程池饱和基础实验 | Java 21 已验证 | 教师参考证据（提交后解锁） |
| 真实 ThreadPoolExecutor 拒绝路径和自定义 JFR 事件 | JDK 21 已验证 | 教师参考证据（提交后解锁） |
| k6 开放/封闭/分阶段恢复脚本 | 已准备，静态验证 | 教师参考证据（提交后解锁） |
| NotifyFlow Micrometer 指标实现 | 已准备，运行 Pending | 教师参考证据（提交后解锁） |
